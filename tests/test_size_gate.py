"""Min-order-size gate tests (Phase 5, 2026-04-28).

Polymarket CLOB rejects orders below 5 shares OR $1 notional.  Two
gates run in series: one inside ``compute_size`` (BUY path) and one
inside ``Executor._execute_one`` (SELL path).  These tests pin the
boundaries on both paths so a future tweak to defaults / rounding
doesn't silently re-introduce the rejected-order retry storm.
"""
...
class _FakeClob:
    ...
    ``strategy.min_order_size_shares`` off it via getattr()."""
...
@pytest.mark.asyncio
async def test_sell_below_min_shares_skips_clob_call():
    """4 shares < 5 share floor → executor must not POST to CLOB."""
    ...
    # Insert an open position with 4 shares (the SELL would target this).
    await store.insert_position(
        event_id="ev_sell", token_id="tok_sub", token_type="NO", city="Chicago",
        slot_label="80°F to 84°F", side="BUY", entry_price=0.45, size_usd=1.80,
        shares=4.0, strategy="D", buy_reason="seed",
    )

    sig = _sell_signal_with_token("tok_sub", price_no=0.40)
    await executor.execute_signals([sig])

    # CLOB never hit; one SKIP row in decision_log.
    ...
    assert len(rows) == 1
    assert "SIZE_BELOW_MIN_SHARES" in rows[0]["reason"]
    await store.close()
