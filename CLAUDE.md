...
- **Three strategy variants** (B / C / D) running side-by-side as a `max_no_price` control comparison.  All three share kelly_fraction=0.5, locked_win_kelly_fraction=1.0, min_no_ev=0.05, max_exposure_per_city_usd=$10, max_positions_per_event=4, max_position_per_slot_usd=$5; only `max_no_price` differs:
  ...
  - Per-city cap dropped from $20 (B-only era) to $10 each so the three variants together cap at $30/city — 1.5× the prior B-only ceiling.  This is intentional: the relaxed-cap variants need room to fire, but the global $200 total exposure ceiling and `daily_loss_limit_usd=50` daily breaker remain in force (tightened 2026-04-28 to match the actual $200 live wallet — was 1000/75 sized against a notional cap that wasn't enforced).
...
- **Global StrategyConfig defaults**: `max_total_exposure_usd=200`, `daily_loss_limit_usd=50`, `locked_win_max_price=0.90`, `exit_cooldown_hours=4`
