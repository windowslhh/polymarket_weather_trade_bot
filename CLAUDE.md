...
- Locked-win 0.95 price cap: removed by Fix 2 (035d353), **reinstated 2026-04-17** as `StrategyConfig.locked_win_max_price` (default 0.95, defined in `src/config.py`, surfaced in `config.yaml`). Initially landed as a module constant; promoted to a config field same day (review #3) so future tuning doesn't require code change + redeploy. The Fix 2 rationale (let `ev > 0` gate filter alone) failed in production — paper→live slippage exceeded the razor-thin EV at 0.997+. Hard cap now runs *in addition to* the `ev > 0` safety net; Fix 2's below/above-lock win_prob split (0.999 / 0.99) is preserved within [min_no_price, 0.95].
