...
- **Polymarket Weather taker fee = 5%** (post 2026-03-30 rollout, `TAKER_FEE_RATE` in `src/strategy/gates.py`). Formula: `fee_per_dollar = TAKER_FEE_RATE * price * (1 - price)` — peaks at p=0.50 at 1.25¢/$ and decays toward the extremes. **No ×2 factor** (a long-standing pre-FIX-2P-2 bug doubled the formula and used 1.25%, computing roughly half the true fee). Both the strategy gates and the backtest engine read this constant; preflight (FIX-2P-6) cross-checks against the broker's reported rate on startup. All backtest reports prior to 2026-04-26 (FIX-2P-2 commit) overstate net PnL.
...
- Forecast cache `_cached_forecasts_by_date` is keyed in **city-local time** (FIX-2P-3, 2026-04-26). Each city contributes to the date keys derived from its OWN `ZoneInfo(city.tz)`, not UTC. H-9 was the symptom of UTC-keying — between 00:00 and 08:00 UTC, west-coast cities are still on yesterday-local; the UTC-keyed cache held tomorrow's-from-our-perspective forecast under "today" while the event's `market_date` (city-local from `discovery.py:229`) pointed at yesterday. The lookup `_cached_forecasts_by_date[market_date][city]` then missed and the rebalancer fallback routed a wrong-day forecast into the evaluator, tripping FIX-22's invariant and crashing position_check 7×/25h in paper. Use `src/weather/forecast.py::city_local_date(city, offset_days=N)` whenever you need a city's local "today + N" anchor; never re-introduce `datetime.now(timezone.utc).date()` as a forecast cache key.
...
- Whitelist REJECT code is `CITY_NOT_IN_WHITELIST` (UPPERCASE) per FIX-2P-11 — pre-fix the rebalancer emitted lowercase `city_not_in_whitelist`, which broke decision_log GROUP BY queries that key off the SCREAMING_SNAKE convention shared with GATE_MATRIX codes.
- `place_limit_order` calls `create_and_post_order(OrderArgs(...))` — pre-FIX-2P-1 it passed a snake_case dict that crashed the SDK signer with AttributeError on the first live order (paper mode early-returns above the SDK call, masking the bug across 25h of paper smoke testing).
