...
> **⚠️ 部分回退于 2026-04-17**: 价帽 `price > 0.95` 已被加回（`LOCKED_WIN_MAX_PRICE` 常量），
> 因为生产数据证明 `ev > 0` 单门槛不足以过滤 paper→live slippage 主导的薄边际带。
> below/above-lock win_prob 分流 (0.999 / 0.99) 保留。
