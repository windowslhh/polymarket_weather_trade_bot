...
`StrategyConfig` 新增字段(初版作为模块常量 `LOCKED_WIN_MAX_PRICE` 落地,
review #3 当天即提升为 config 字段,以便后续不重新部署即可调整):
...
`ev <= 0` 安全网那条同样补了 debug-log (review #2),两条 skip 路径在 paper 日志里都可见。
...
这部分语义本身是对的，只是触发范围被压缩到 `[min_no_price, locked_win_max_price]` (默认 `[0.20, 0.95]`)。
...
### 新增 (`TestLockedWinPriceCap` class, 8 tests)
| 测试 | 场景 | 期望 |
|------|------|------|
...
| `test_default_config_value` | `StrategyConfig().locked_win_max_price == 0.95` | passes |
| `test_config_override_actually_takes_effect` | cap=0.85 → 0.90 reject, 0.80 accept | proves config wired |
| `test_config_override_can_relax_cap` | cap=0.99 → 0.97 accepted | proves config wired |

(`test_default_config_value` + 两个 override 测试取代了原 `test_constant_value` —
后者只做 literal 比较,新版同时验证默认值 + config 实际被读到。)
...
(Fix 2 baseline 792；本次净 +8 = 6 boundary tests + 3 config-aware tests
− 1 Fix 2 高价 accept 测试 = +8。)
...
| 文件 | 类型 |
|------|------|
| `src/config.py` | `StrategyConfig.locked_win_max_price: float = 0.95` 字段 |
| `config.yaml` | `strategy.locked_win_max_price: 0.95` (含说明注释) |
| `src/strategy/evaluator.py` | `evaluate_locked_win_signals` 读 `config.locked_win_max_price` 做硬过滤;`ev <= 0` 分支补 debug-log |
